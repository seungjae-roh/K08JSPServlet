...
public class PassController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		/*
		/mvcboard/pass.do?mode=edit&idx=121
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
		...
		if (confirmed) {
			if (mode.equals("edit")) {
				...
				session영역은 페이지를 이동해서 데이터가 공유되므로 해당
				게시물을 수정 완료할 때까지 유지하고, 수정이 완료되면 제거할
				것이다. 여기에서 저장한 패스워드는 수정을 위한 update
				쿼리문의 where절에서 사용할 예정이다.
				 */
				...
				검증없이 진입하였다며 세션에 저장된 패스워드가 없을 것이므로
			...
			JSFunction.alertBack(resp, "비밀번호 검증에 실패했씸");
		}
	}
}
