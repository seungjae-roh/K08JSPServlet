...
<body>
	...
	완전히 서로 다른 페이지를 의미한다. 따라서 page영역, request영역
	모두 공유되지 않는다. -->
	<%
	...
	최초 요청한 페이지의 경로가 보여지지만, 사용자는 포워드된
	페이지의 내용을 보게 된다. 즉, 하나의 요청을 2개의 페이지가
	공유하는 개념이다.
	 -->
	<h2>액션 태그를 이용한 포워딩</h2>
	<!-- HTML주석에 아래의 코드(JSP 코드)를 넣으면 주석처리 되지 않고 그대로
	실행된다.
		- HTML주석: 화면에 표시하지 않기 위한 주석
		- JSP주석: 실행되지 않게 하기 위한 주석 
	-->
	<jsp:forward page="/07ActionTag/ForwardSub.jsp"/>
	<%
	/* 액션태그를 사용하면 JSP보다 훨씬 간결하게 코드를 표현할 수 있다. */
